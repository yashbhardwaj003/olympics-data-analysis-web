...
def fetch_medal_tally(df, year, country):
    medal_df = df.drop_duplicates(
        subset=['Team', 'region', 'NOC', 'Games', 'Year', 'Season', 'City', 'Sport', 'Event', 'Medal'])
    flag = 0
...
def medal_tally(df):
    medal_tally = df.drop_duplicates(
        subset=['Team', 'NOC', 'region', 'Games', 'Year', 'Season', 'City', 'Sport', 'Event', 'Medal'])
...
def most_successful(df, sport):
    ...
    return temp_df['Name'].value_counts().reset_index().head(20)
...
def yearwise_medal_tally(df,country):
    ...
    final_df = new_df.groupby('Year').count()['Medal'].reset_index()  # year wise medal count
...
def most_successful_cw(df, country):
    temp_df = df.dropna(subset=['Medal'])
